...
if __name__ == '__main__':
    ...
    for i in range(opt.epoch):
        ...
        torch.save(poolingnet.state_dict(), os.path.join(opt.outputs_dir, 'lowpass/epoch_{}.pth'.format(i)))
    ...
    torch.save(best_weights, os.path.join(opt.outputs_dir, 'lowpass/best.pth'))
    ...
    optimizer2 = torch.optim.Adam(unet.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    scheduler2 = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizer2,milestones=[1600],gamma=opt.gamma)
    ...
    for i in range(opt.epoch):
        ...
        for batch_idx, (gtBatch, msBatch, panBatch) in enumerate(train_loader):
            ...
            highpass = unet(panBatch - gaussian_conv_1(panBatch), msBatch - gaussian_conv_4(msBatch))
            lowLoss=loss(highpass,gtBatch - gaussian_conv_4(gtBatch))
            lowLoss.backward()
            optimizer2.step()
            epoch_losses.update(lowLoss.item(), msBatch.shape[0])
            batch_time.update(time() - end)
            ...
            if (batch_idx + 1) % 100 == 0:
                ...
                print(training_state)
        print('%d epoch: loss is %.6f, epoch time is %.4f' % (i, epoch_losses.avg, batch_time.avg))
        torch.save(unet.state_dict(), os.path.join(opt.outputs_dir, 'highpass/epoch_{}.pth'.format(i)))
        unet.eval()
        epoch_SAM=functions.AverageMeter()
        with torch.no_grad():
            for j, (gtTest, msTest, panTest) in enumerate(val_loader):
                if torch.cuda.is_available():
                    ...
                    gtTest = Variable(gtTest.to(torch.float32))
                msTest = torch.nn.functional.interpolate(msTest, size=(256, 256), mode='bilinear')
                highpass = unet(panTest - gaussian_conv_1(panTest), msTest - gaussian_conv_4(msTest))
                test_SAM = functions.SAM(highpass, gtTest - gaussian_conv_4(gtTest))
                if test_SAM==test_SAM:
                    epoch_SAM.update(test_SAM,highpass.shape[0])
            print('unet eval SAM: {:.6f}'.format(epoch_SAM.avg))
        if epoch_SAM.avg < best_SAM:
            best_epoch = i
            best_SAM = epoch_SAM.avg
            best_weights = copy.deepcopy(unet.state_dict())
        print('best epoch:{:.0f}'.format(best_epoch))
        scheduler2.step()
    print('unet best epoch: {}, epoch_SAM: {:.6f}'.format(best_epoch, best_SAM))
    torch.save(best_weights, os.path.join(opt.outputs_dir, 'highpas/best.pth'))
